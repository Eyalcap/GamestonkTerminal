...
def get_suppliers(ticker: str) -> str:
    ...
    text_supplier_chain = BeautifulSoup(requests.get(url_supply_chain, timeout=60).text, "lxml")
...
def get_customers(ticker: str) -> str:
    ...
    text_customer_chain = BeautifulSoup(requests.get(url_customer_chain, timeout=60).text, "lxml")
